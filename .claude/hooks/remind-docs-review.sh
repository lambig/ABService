...
#      → バーンダウン規約（完了項目は削除・作業ログ禁止）に加え、
#        変更したdoc以外のストックドキュメント（他のdocsやREADME等、相互参照・
#        重複言及している箇所）が今回の変更で陳腐化していないかの見直しを促す。
