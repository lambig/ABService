 ...
 * - `admin`: scratch の作品・記事を作って消す、サイト文言を書き換える、セッションを失効させる、応答を
 *   遅らせる。scratch とサイト文言のキーは worker ごとに分かれている（`src/support/worker.ts`）が、
 *   一覧の1ページ目に自分の行があること・記事の総数・シード済みの行の位置を前提にする spec が残るため、
 *   まだ 1 worker で直列に走らせる（#430）。
 *
