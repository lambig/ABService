...
- **PMD**: errorprone/collection 系。※ **Java 25 対応版の可否を要確認**（SpotBugs は Java 25 非対応で無効化済みのため、PMD も同様の懸念がある）。
- **カスタム**: 上記で表現できない意味論的ルール（値を返す `if` の禁止 等）。実装コスト高。
- **適用不可（N/A）**: Kotlin 固有機能に依存し Java に対応概念が無いルール。

さらに、**Kotlin 由来のイディオム的ルールをそのまま Java に持ち込むと可読性を損なうもの**（論理演算子の関数化など）については、「ハード強制」か「レビュー観点（ガイドライン）に留める」かを個別に判断する。

### 既存コードへの影響（重要）

いくつかのルール（論理演算子禁止・値を返す `if` 禁止 等）は、**現行 ABService コードのほぼ全体が違反**している。これらを機械強制するには、既存コードの大規模改修または baseline 化が必要で、単なる「チェック追加」では完了しない。方針として:

- **ratchet（漸進）方式を基本**とする: 新規・変更コードにのみ強制し、既存違反は baseline に記録して段階的に解消する。
- **全体一括改修（big-bang）は原則採らない**（レビュー不能な巨大差分・回帰リスクのため）。
- 違反が広範なルールは、投資前に「採用/不採用」を確定する。
...
| # | ルール（意図） | Java強制手段 | 採否 | 既存違反 | フェーズ |
|---|---|---|---|---|---|
| 16 | 論理演算子 `&&`/`\|\|`/`!` 禁止（宣言的 `and/or/not` へ） | Checkstyle `IllegalToken` + ヘルパlib | 📝 or 判断 | **広範** | P5 |
| 17 | 値を返す `if` 禁止（値位置の if・値を返すガード節）→ `switch` 式へ | カスタム（Checkstyle 近似は困難） | 📝 or 判断 | **広範** | P5 |
| 18 | 値を返さない位置での `switch`（`when`）禁止 | カスタム | 📝 | 要確認 | P5 |
| 19 | VO の `fromInput` にバリデーション必須（VO化尚早シグナル） | カスタム | 📝 | — | P5 |

> #16/#17 は元々本番・テスト双方に適用されるルールだが、Java では可読性・イディオムの観点で「そのままハード強制」が最適とは限らない。**採用形態（ハード強制 / レビュー観点）を要判断**（§4 決定事項）。
...
| **P5** | 判断が要るルール（#14–#19）。採用形態決定後に着手 | §4 の決定 | 高（既存改修 or baseline） |

各フェーズは独立ブランチ・PR で進め、pre-commit フックと CI（`check`）で回帰を防ぐ。ArchUnit/Checkstyle 設定変更は品質ゲート＝ポリシー変更のため、都度承認のうえ実施する。

---

## 4. 未決事項（要判断）

1. **判断ルール（#16 論理演算子 / #17 値を返す if 等）の採用形態**: Java でも「ハード強制」するか、「レビュー観点（ガイドライン）」に留めるか。既存コードが広範に違反しており、ハード強制なら大規模改修 or baseline が前提。
2. **既存違反の扱い**: ratchet/baseline（新規のみ強制・段階解消）か、対象ルールごとの一括改修か。
3. **PMD 再導入の可否**: Java 25 対応版が使えるか要調査。使えない場合は Checkstyle + ArchUnit + カスタムで代替する。
4. **進行中 PR の扱い**: `fromInput`（A-4）は #16/#17 に違反する実装になっている。先行して規約準拠へ直すか、P5 の全体判断まで現行イディオム（既存コードと同様）で据え置くか。
