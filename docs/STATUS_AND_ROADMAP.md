...
見た目は配色と書体の実値までトークンに入っている（DECISIONS 25）。公開サイトは外部から書体を読み込み、届くまで画面を覆う。サイトの印は公開サイトが持ち、リンクプレビューの既定画像はその印から組み立てのたびに焼く——どのページも、作品から採る画像が無ければこの既定になる。印は既定であって正式な意匠ではない（正式版は運用側で決める。#375）。管理画面は印を持たない。ページの末尾はコピーライト表示だけを持ち、主体はデータ側、年と記号は画面が組み立てる（静的出力のため、年は次に作り直すまで変わらない）。ページ送りは、たどれない側を出さずに場所だけ空ける。
