...
- **409**（業務ルール違反・競合）: **返し得ると宣言した操作だけ**（`MayConflict`）。HTTP メソッドから導くと、資源を書き換えない操作（署名付きURLの払い出しなど）にまで広がる
...
**トレードオフ**: 400 / 404 / 500 の範囲は定義の形（パラメータの位置・本体の有無）から導くが、**409 だけは形から導けない**。業務ルール違反を返すかは実装の分岐の中にあり、注釈も戻り値型も持たないため、返し得る操作を名指しする宣言に頼る。宣言は「エンドポイントごとに書かない」という原則の例外であり、次の非対称を伴う。

- 宣言した操作が実際に 409 を返すことは統合テストが固定する（15 操作すべて）
- **宣言を忘れた操作は静的には検出できない**。定義に 409 が出ないだけで、実装は 409 を返し続ける
- 基盤由来の楽観ロック（`OptimisticLockException`）はどの更新にも潜在するが、これを根拠に全更新へ広げると元の「HTTP メソッドで一律」に戻るため含めない。宣言の対象は明示的な経路（業務ルール違反・`expectedRevision` による世代照合）に限る
...
**実体**: `ProblemDetailErrorContract`（マッパーの数え上げと宣言の集約）、`ProblemDetailResponseFilter`（範囲の判定と反映）、各 `*ExceptionMapper` の `@APIResponse`、`MayConflict`（409 を返し得る操作の宣言）、`DeclaredEndpoints`（宣言から操作を同定する）、`LayeredArchitectureTest`（数え上げ漏れの検出）、`OpenApiSchemaRestIntegrationTest` の `errorResponsesCarryProblemDetail` / `unreachableErrorsAreAbsent`、`AlbumRestIntegrationTest` の `malformedIdIsNotFound`（400 の範囲の根拠となる挙動）。
