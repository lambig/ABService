...
**タグは commit ごとに1つ（`sha-<full SHA>`）だけ発行し、ホストへは digest で配る**。ECR の保持規則は接頭辞でタグを選ぶため、発行する接頭辞と保持する接頭辞がずれると、押し込んだイメージは数に入らないまま溜まり続ける（気付くのはストレージの請求）。規則を1つにし、両者が揃っていることを CI が突き合わせる。移動するタグ（`latest`）は発行しない——どの実体を指すかが時点で変わるタグは、保持の数え方も証跡も曖昧にし、配布は常に明示のタグを使うため要らない。ホストへ渡す参照を digest にするのは、リポジトリが MUTABLE（同じ commit の Deploy を再実行すると同じタグを押し直す）でも、配った時点の実体を一意に残すため。IMMUTABLE にすれば押し直しを拒めるが、再実行の経路に例外処理が要る。実体の固定は digest で得られるので、そちらを採る。
