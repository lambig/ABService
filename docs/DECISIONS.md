...
**ブラウザ側の移行契約（#264の後続実装）**: トークンと期限はタブごとの `sessionStorage` に保持し、元のキーを保存しない。ページ移動・リロードでは期限内のセッションを再利用し、独立した新規タブは再認証する。タブ複製等でstorageが複写されれば同じセッションとして扱い、一方のサーバ失効で他方も次の要求が401になる。タブ間で元のキーやトークンを配る共有経路は設けない。
...
**移行状況**: 今回はバックエンドの交換・認証・失効を実装する。管理画面の `credentials.ts` はまだAPIキーを `sessionStorage` に保持しており、フォーム・保持・要求世代・E2Eを移行するまで #264 は未完了。以前の「localStorageにキーを保持する」という記録は現行実装と一致しないため廃止する。

**実体**: `presentation/rest/security/AdminSessions`・`AdminSessionResource`・`ApiKeyIdentityProvider`、認証キーの設定は `abservice.auth.admin-api-key`。DBスキーマ・追加秘密設定・CloudFrontのcookie転送は変更しない。
