...
**トレードオフ**: 生のパレットを一時的に使いたい場面で不便になる。配色を決めていない段階から、ダークの2組を維持する手間が発生する。公開サイトは書体の取得が済むまで OS の標準で描画されるため、字面が一度入れ替わる。和文の書体は軽くないので、初回表示の重さを引き受ける（軽量化は公開後に扱う）。

**実体**: `frontend-public/src/styles/global.css` と `frontend-admin/src/styles/global.css`（トークンの実値と書体）、`frontend-public/src/layouts/BaseLayout.astro`（書体の読み込み）、#228（クラスを出力しない描画）。
