...
**子の識別は親の中でしか意味を持たない。** 要求が子をIDで指すとき、そのIDがその集約の子であるかを確かめられるのは集約自身だけである。外から受け取った識別子で子を組み立ててから集約へ渡す形にはしない——集約が所有関係を確かめないまま受け取ると、要求元が選んだ識別子を持つ子を作れてしまう（永続化は、知らないIDを新しい行として受け取る）。同じ子を1つの並びに2度置くこともできない。

**なぜ**: 「アルバムのトラック構成」は一貫性を1トランザクションで守りたい単位そのものであり、1アルバムのトラック数は通常10〜20曲程度で集約サイズが問題にならない。

外から来たIDの扱いも同じ理由に立つ。子が親の外に存在できない以上、「このIDは存在するか」という問いは親を決めないと答えられない。形式が正しいことと、その子がここに属することは別の判定である。

**見直し条件**: トラック数の増大やロック競合でパフォーマンス問題が出た場合、Track を別集約へ分離する。

**実体**: `domain/model/aggregate/album/`（Album / Track / TrackTune）、`AlbumRepository` のみ存在すること。`Album#replaceTracks` / `Album#replaceExternalAudios`（所有関係と重複の判定）、`Track.Row` / `ExternalAudio.Row`（集約へ渡す、番号を持たない行）、`AlbumChildOwnershipRestIntegrationTest`（未知ID・別作品のID・重複・作成時のID指定）。
...
管理画面の作品名義は、共通のサークル名と異なる場合だけ入力する。省略時は保存時点の `site.artist` を管理クライアントが補完し、API/DBには実効名義を保存する。共通名義が未設定・取得不能なら保存を止め、設定を案内する。明示された名義と既存作品の保存値は共通名義の変更で書き換えない。初出イベントの頒布サークル名は委託等で異なるため、この補完に使わない。
...
- 作品が何のアレンジ集なのかは、**作品（Album）が人の書いた一文として1つ持つ**（`originalWorkNote`）。作品名を構造として持ち、表示側で「より各曲」と組み立てる形は採らない
...
出典を一文で持つのは、**言い回しそのものが作者の主張**だから。「「○○」より各曲」と「「○○」のメインテーマのみ」は、同じ作品名から組み立てられない。作品名だけを構造として持つと、どちらかの言い回しを画面が選ぶことになり、選ばなかった側の盤で嘘になる。

**トラック側には置き場を作らない。** トラックと原曲が N-N で、かつその対応を**特定しない意図がある**盤がある。`TrackTune` の原曲情報をその代わりに使ってはならない——置き場があると空欄が「未入力」に見えて埋める圧力になり、埋まった瞬間、作者が述べていない対応をシステムが主張したことになる（#89）。「特定しない」という意図は、作品側の記述が**あること**が運ぶ。

出典は**同定のキーにしない**。1つの作品を指すとは限らず、綴りで寄せると別々の作品が1つになる（#183 のチューンの同定と同じ理由）。構造化の要否は、原作を横断して引く機能を作る時点で判断する。

**実体**: `domain/model/aggregate/album/Track` / `TrackTune`、`domain/model/aggregate/tune/Tune`、`domain/model/vo/album/OriginalWorkNote`、`V4`〜`V6`、`V46`。
...
**ビルドする架構を実機に合わせ、実機のツールは版を固定する**。イメージは arm64 のランナーで作る。本番の EC2 が arm64 のため、amd64 で作ると実機が動かせない manifest が ECR へ入り、気付くのはデプロイの最中になる（押し込む前に架構を確かめる）。jar はランナー上の Gradle で組み、`Dockerfile.jvm` はそれを包むだけにする。コンテナの中で Gradle を回すと依存の取得からコンパイルまで毎回冷えた状態でやり直すことになり、ランナー側の build cache が効かない。uber-jar は JVM のバイトコードで架構に依らないため、どこで組んでもイメージの架構は変わらない。`docker compose` は AL2023 のリポジトリに無いため `user_data` が公式の手順どおりプラグインの置き場へ入れる。版を固定したうえで、**ダイジェストをリポジトリ側に持って突き合わせる**。版の名前だけでは実体が決まらず（GitHub の release は差し替えられる）、配布物と一緒に置かれた `.sha256` を使う形では、両方を差し替えられたときに突き合わせが素通りするため。インスタンスを作った時期だけで実機の compose が変わらないようにする意図（8 と同じ理由）は、bytes まで固定してはじめて満たせる。
...
**タグは commit ごとに1つ（`sha-<full SHA>`）だけ発行し、一度押し込んだら動かさない**。ECR の保持規則は接頭辞でタグを選ぶため、発行する接頭辞と保持する接頭辞がずれると、押し込んだイメージは数に入らないまま溜まり続ける（気付くのはストレージの請求）。規則を1つにし、両者が揃っていることを CI が突き合わせる。移動するタグ（`latest`）は発行しない——どの実体を指すかが時点で変わるタグは、保持の数え方も証跡も曖昧にし、配布は常に明示のタグを使うため要らない。

同じ commit のイメージは二度は作らない。ビルドはベースイメージやパッケージの取得を含み、同じ commit でも同じ実体にはならない。作り直してタグを押し直すと、以前その commit で配った実体は untagged になって消え、後日の手動ロールバック（commit を指定する）が戻すのは「当時の実体」ではなく「タグが今指す実体」になる。そこで配布は、その commit のタグが在れば通常経路でもビルドと push を飛ばして在るものを使い、リポジトリは IMMUTABLE にして他の経路からもタグを動かせないようにする。ロールバックが戻すのは**その commit に最初に配った実体**で、戻せる範囲は保持規則が残す直近10件。期限切れで消えた後に同じ commit を配り直せば、それは新しい実体になる。ホストへ渡す参照は digest にし、配った実体をログと Summary に一意に残す（証跡の「稼働 image digest」の出所）。
...
## 22. 管理APIキーを期限付きBearerへ交換する

**判断**: ブラウザは入力したAPIキーを交換にだけ使い、その後は30分で失効する不透明Bearerトークンを使う。固定期限とし、利用やトークンからの再交換で延長しない。初期投入等の管理API利用者は引き続きAPIキーをBearerで使える。公開サイトのSSGは匿名の公開Query APIを読み、管理APIキーを渡さない（公開データ世代と配布の契約は `infra/release/README.md`）。

**サーバ契約**:

- `POST /api/v1/admin/sessions`: 元のAPIキーで認証して `{ token, expiresAt }` を200で返す。`Cache-Control: no-store` を付ける。セッションからの交換は403、誤ったキーは401。
- トークンは暗号学的乱数256 bit。サーバにはSHA-256 digestと期限だけを保持する。
- `DELETE /api/v1/admin/sessions/current`: 呼出元セッションを失効させ204を返す。失効後の提示は401。別セッションを巻き込まない。APIキーでの呼出しは403。
- 単一backendプロセスのメモリに最大128セッションを保持する。上限では最古のセッションを失効させる。同期限のものの順序は保証しない。再起動・再配布で全セッションが失効するため再認証が必要。
- キーの更新は設定更新とbackend再配布を組にして行う。SSMの値を書き換えただけでは稼働プロセスのキーは変わらない（順序は「資格情報の更新は tfvars の契機で生成し直し、直後に再配布する」）。複数backendへ拡張するときは共有セッションストアが必要。
- 失効は以後の認証を拒む。既に認証され実行中のCommandを取り消す機能ではない。

**ブラウザ側の契約**: トークンと期限はタブごとの `sessionStorage` に保持し、元のキーを保存しない。ページ移動・リロードでは期限内のセッションを再利用し、独立した新規タブは再認証する。タブ複製等でstorageが複写されれば同じセッションとして扱い、一方のサーバ失効で他方も次の要求が401になる。タブ間で元のキーやトークンを配る共有経路は設けない。

破棄はブラウザ側のセッション世代を先に進めて保存を消し、そのトークンをサーバでも失効させる。失効通信が失敗した場合、サーバ失効済みとは表示しない（最長30分の期限は残る）。古い要求の完了で保持情報や認証済み画面を復活させず、再認証後に古い401が届いても新しいセッションを破棄しない（#122）。交換処理にも同じ規則を適用し、入力キーを成功・失敗の双方でフォームから消す。

**なぜこの方式か**: 既存のAuthorization転送を使え、APIキーそのものの継続保持を止められる。自動送信されるcookieは採用せず、cookie転送・SameSite・CSRFを追加しない。現在のタブ単位の操作を維持する代わりに、独立した新規タブの再認証を受け入れる。

**XSSに関する制約**: `sessionStorage` のトークンも同一オリジンのスクリプトから読める。短い期限と失効は露出後の有効期間を縮めるもので、XSSを防ぐものではない。公開側と管理側は同一オリジンのため、描画経路とCSP（#240）の検査は継続する。HttpOnlyを採る場合も認証済み操作の実行までは防げず、入力中の元キーもXSSから保護できない。利用者投稿・外部テキスト等へ描画対象を広げる時点で保持方式とオリジン分離を再評価する。

**実体**: `presentation/rest/security/AdminSessions`・`AdminSessionResource`・`ApiKeyIdentityProvider`、認証キーの設定は `abservice.auth.admin-api-key`。ブラウザ側は `frontend-admin/src/lib/credentials.ts`・`api/sessions.ts`・`ApiKeyForm.svelte`・`SessionControls.svelte` と各画面の応答反映処理。DBスキーマ・追加秘密設定・CloudFrontのcookie転送は変更しない。
...
- **画像は埋め込めるが、`src` は配信ベースパス配下に限る**。それ以外の `src` は描画時に落とす。#289 では WHATWG URL 解決後の origin・pathname 境界まで検査する。dot segment・backslash による正規化が必要な曖昧な入力は書き換えず拒否し、Markdown が符号化する `%5C` も拒否する。配信ベースは root-relative path（`/` 自体は不可）とし、不正設定時は画像を許可しない。query・fragment を持つ正常な画像は従来どおり許可する
- **描画は共有パッケージの純粋関数1本**とし、公開サイトはビルド時、管理画面のプレビューはブラウザで、同じ関数を呼ぶ

**なぜ**: 22 のBearer保持方式でも「公開ページが描画する文字列に実行可能なものが混ざらない」ことが必要になる。生HTMLをパースする経路を持たなければ、この前提は**入口で閉じ**、描画側フィルタの網羅性に依存しない。
...
**書体の取得は公開サイトと管理画面で分ける**: 公開サイトは書体を読み込んで字面を固定し、管理画面は OS の標準に委ねる。公開サイトは作品を見せる場であり、閲覧環境ごとに字面が変わることを避ける価値が読み込みの重さに見合う。管理画面を見るのは運営者だけで、字面を固定して得るものが無い。読み込み先を外部に取るため、公開サイトの CSP は書体の取得元を許す必要がある（#240）。

**トレードオフ**: 生のパレットを一時的に使いたい場面で不便になる。配色を決めていない段階から、ダークの2組を維持する手間が発生する。公開サイトは書体の取得が済むまで内部では OS の標準で描画されるため、**その入れ替わりを見せないよう、届くまで画面を覆う**（#361）。覆いには待ち時間の上限があるので、上限を越えて届いた場合だけ入れ替わりが見える。和文の書体は軽くないので、初回表示の重さを引き受ける（軽量化は公開後に扱う）。

**画像はトークンを読めない**: サイトの印（#341）は色の実値を自身に持つ。これは機構の例外ではない——閉じているのはクラスとして解決される経路であり、画像はそこに乗らない。逆に、印の側からトークンを参照する道は無いため、トークンを変えたら印も変える対象になる。

**実体**: `frontend-public/src/styles/global.css` と `frontend-admin/src/styles/global.css`（トークンの実値と書体、覆いの見た目）、`frontend-public/src/layouts/BaseLayout.astro`（書体の読み込みと覆いの出し外し）、`frontend-public/public/favicon.svg`（印。色の実値を持つ理由もここ）、#228（クラスを出力しない描画）。
...
フロントに code から field への対応表を置くと、検証を1つ足すたびに2箇所を変える必要が生まれ、片方だけ古くなっても画面は動いてしまう（誤った欄へエラーが出る）。
...
## 30. 全項目置換の更新は、編集を始めた時点の世代を条件に取る
...
**編集単位は、全項目置換と競合の判定が守るべき整合性の範囲で決まる。その範囲を書く経路は1つに寄せる。** 経路が複数あると、片方で書くたびに、もう片方が抱えている書きかけを守る手当てが要る。何がその範囲を変えるかは対象ごとに違い、**契約ではなく当てはめの問題**として実測で確定する。

- アルバム: `Track`・`TrackTune`・`ExternalAudio` はアルバムの外に独立して存在できず、置換の単位も競合の単位もアルバム全体である。したがって**集約全体が1つの編集単位**で、書く経路も集約ルートに1つだけ置く（#391）。子だけを直した保存も作品の更新として世代を進める。進めるのは親の列を汚すことによる——子のコレクションは `mappedBy` の逆側で、増減しても親行は汚れないため、何もしなければ世代が動かない
...
**なぜ**: 全項目置換の更新は、届いた値をそのまま最新の状態へ適用する。2つのタブが同じ詳細を読み、片方が保存した後にもう片方が「別の項目だけ直した」つもりで送ると、読んだ時点の値が最新を上書きする。この経路は DB の版でも行ロックでも検出できない——後の要求は最新を読み直してから更新するため、競合そのものが起きていない。検出できる位置は「読んだ時点」を要求が持ち込んだときだけである。

未指定を通すと、条件を送らないだけで後勝ちに戻れる抜け道が残り、機構で守れない。

アルバムでは「重ならない編集」という状態が画面にも現れない——1回の保存が作品と曲目を同時に送るためである。むしろ進めない側に穴が開く。2つのタブが別々のトラックを直しても競合にならず、後の保存が前の変更を全項目置換で消す。記事のタグを別の単位に置くのは、それが記事そのものの更新ではないから（上記）であって、経路が分かれているからではない。経路の形は編集単位の結果であり、根拠ではない。
...
**トレードオフ**: イメージを作るために backend をもう一度組む（`quarkusBuild`）ため、CI の総実行時間が増える。他のジョブと並列に走るので待ち時間は変わらない。jar はランナー上の Gradle で組み、コンテナの中では回さない（16）。
...
**トレードオフ**: 値を切り替える瞬間、配信と backend のどちらかが古い値を持つため断が生じる。新旧を同時に受け付ける形は持たせていない。更新の順序は「資格情報の更新は tfvars の契機で生成し直し、直後に再配布する」。
...
## 編集途中の復旧はタブ内のスナップショットにする

作品・記事の入力には、必須欄の空文字や組み立て途中の曲目など、まだ保存できない状態がある。これを集約の自動更新で扱うと、不変条件の検証と編集開始世代による競合制御（30）に干渉する。#308 はブラウザ内の復旧スナップショットとし、サーバーへの更新は既存の明示保存だけにする。

v1.0は `sessionStorage` を選ぶ。同じタブでのリロード・誤遷移に備え、閉じた後の長期保存は持たない。`localStorage` や IndexedDB が必要とする長期残存・期限・複数タブ間調停を導入しないためである。タブ復元やクラッシュ後の復旧はブラウザ依存であり保証しない。

画面の種類と対象（新規作成は専用の識別）を保存キーと内容の両方で照合する。入力値は認証状態から分離し、既知の欄だけを読み戻す。利用者が復元・破棄を選ぶまで既存の退避を保持する。退避世代と現在の世代が違う場合は入力を保持して競合を示し、最新の世代への自動置換やPUTを行わない。保存成功・明示破棄・最新読み直しで対象の退避と保留タイマーを消し、明示ログアウトでタブ内の退避全体を消す。認証切れは入力の破棄とは扱わない。


## 下書きプレビューは管理画面で公開レイアウトを共有する

#214 は公開APIの非公開404契約を変えず、編集中の入力を管理画面内で描画する。本文の変換だけを共有しても、見出し・作品参照・外枠が別実装なら公開時の配置を確認できないため、それらと公開CSSを `packages/public-presentation` に集める。Astroはビルド時、管理画面はブラウザで同じ描画関数を呼ぶ。

管理画面のCSSを公開レイアウトへ混ぜないため、表示先を同一オリジンの空のiframeとする。親だけが管理APIを読み、描画済みHTMLを渡す。プレビュー文書のCSPは script-src を none にし、同一オリジンの文書ではスクリプトを実行しない。sandboxでは別オリジンの試聴プレイヤーに必要なスクリプト実行を許可し、frame-srcをSoundCloudに限定する。記事のリンク遷移は親から止める。下書きや認証情報をURL・静的成果物に入れる経路と、プレビュー専用の公開APIは作らない。

配置は [ba9bed9](https://github.com/lambig/ABService/commit/ba9bed959f869fa2e8db8cc78da51c4aa4aad0ec)（#309）で定めた「左に入力、右にプレビュー」を引き継ぐ。入力中に常時反映し、スクロールに追従、長い本文は枠内スクロール、狭い画面では縦積みとする。段階2はその右側の描画を公開レイアウトへ置き換える。

未保存入力は編集に追従して描画し、保存可能かどうかの判断をプレビューへ持ち込まない。未公開日時は捏造せず省略する。参照作品とサイト文言は最新の取得値を使うため、公開サイトの前回ビルドとの時間差は残る。関連情報の取得失敗では不完全な表示を成功扱いにせず、再試行できるようにする。


## 作品紹介記事には作品ページの内容を展開する

作品ページは作品そのもの、紹介記事はその作品を紹介する公開時点の文章を扱う（19）。ページを分けた意図には、作品ページの内容を紹介記事内へ展開することも含む。#352 / #353ではページ全体のArticle / Workの区別を整理したが、作品の表示を参照リンクとして扱ったため、この意図が実装へ引き継がれていなかった。2026-09-16の仕様確認で次の構成を明示した。

紹介記事の見出し・公開日・タグに続き、**作品の顔（試聴プレイヤー。音源を持たない作品はカバー画像）→ 概要 → 記事本文 → 曲目 → 原作の出典 → 頒布情報（イベント・頒布価格）**を表示する（#415）。記事本文は作品紹介の前置きとして、概要の後ろ、曲目の前に置く。未設定の区画は省略する。作品ページは同じ作品描画を使い、記事本文と頒布価格を持たない（価格の表示先は#349を維持）。

- **顔は見出しの直後、文より先に置く。** 試聴には見出しを付けず、節ではなく絵として扱う。枠は正方形で、一辺は本文幅か 700px の小さい方。本文幅より狭くなるときは本文の中央に置く（絵だけが中央でも、左揃えの文と軸がずれる違和は無い）。プレイヤーは visual 表示にしてアートワークを出す。音源を持たない作品はカバー画像が同じ枠に入り、音源の有無で体裁を変えない。
- **記事内の作品からは、作品の見出し・発売日・品番を出さない。** 記事の見出しが作品を名指しており、同じ大きさの見出しが2つ並ぶと段差が読めない。発売日は記事の公開日と無ラベルで並ぶと見分けられず、品番は作品ページの URL と作品ページが持つ。
- **曲目は畳んで置く**（`details`、既定は閉じる）。長い曲目が本文と頒布情報の間を押し広げないため。開閉はブラウザに任せ、スクリプトを実行しないプレビュー文書でも動く。
- **原作の出典は曲目の直後。** 曲目全体を指す一文で、どのトラックがどれかは述べない（#365）。
- **頒布情報は見出し付きの節で、「日付 イベント名 会場」の行と「スペース番号 サークル名」の行に折り返す。** スペース番号は当日その場で探す値のため強調し、会場は強調しない。価格はイベントの子情報として同じ節の末尾に「頒布価格 1,500円」と置く（通貨は記号でなく名で出す）。サークル名は作品の名義と別に初出イベントが持つ（合同・委託では名義と違う名で出る）。

記事全体はArticle、作品ページ全体はWorkの印で区別し、記事内にもう一つWorkの印を入れない。右側プレビューも同じ描画を使う。入力中は記事の見出しと本文だけを更新し、試聴を再起動しない。タイトル未入力のトラックはサーバーの表示名を使い、編集値のnullを表示名で上書きしない。

## 公開ビルドは保存済み世代の前後照合で受理する

#125の最初の実装は、公開Queryが読むDBの変更を単一行の不透明な世代として追跡する。世代はコードSHA・集約の編集revision・通常配布受理のlast-normalとは独立。保存と同じトランザクションで変わり、commit済みの値だけを匿名Queryが返す。アプリの通知やシーケンス単独では保存と世代の確定がずれ得るため、V47のDBトリガで全参照表のDML/TRUNCATEを捕捉する。子の変更・一括削除・連動非公開化も取りこぼさない。

最小方式として下書きや結果0件の更新も含める。公開に影響するかを全更新経路へ重複実装せず、余分な再ビルドの可能性を受け入れる。UUIDは変更回数を公開せず、値を元へ戻しても過去の成果物を再び最新と誤認しない。単一行への更新は書込みを直列化し、既存の集約ロックとの競合ではDBがトランザクションを取り消す場合がある。大量・高頻度更新向けの設計ではない。

SSG開始前と完了後の世代が一致した成果物だけに、コードSHA・世代・全ファイルのハッシュを付ける。異なる世代を読んだビルドは棄却し、自動で無限再試行しない。取得・応答のキャッシュを使わず、公開データと世代は同じprimary DBを読む。配布開始前とinvalidation完了後も再確認し、不一致・取得不能ではcurrentを進めない。

この方式は「配布完了したデータがどの世代か」の証拠であり、DB保存とS3/CDN更新を原子的にはしない。同期中の新旧混在、最後の確認後の保存による未反映は残る。配布途中の競合はpendingに残して最新データからrecoverする。過去成果物へのrollbackはDBの現世代と同じものだけを許し、撤回前・世代不明のHTMLを復元しない。実AWS上のURL失効・静的404・DB復元を伴う復旧は#125/#130の受け入れに残す。

## 監視はサービスの標準指標と公開 URL の合成監視で組む

#168 の最小構成は、アプリ固有の監視実装を持たず（10 と同じ理由）、サービスの標準指標と、無いものだけを補う形にする。

**ログは agent ではなく Docker の awslogs ドライバで運ぶ。** prod の backend は JSON を stdout へ出しており、ドライバがそのまま 1 行 1 イベントで CloudWatch Logs へ送れる。agent にログも任せるとホストのファイルを経由する段が増え、ローテーションと権限の設定が要る。ドライバは `non-blocking` にし、ログ配送のバックプレッシャー（宛先が遅い・一時的に届かない）ではバッファに溜めて溢れたら捨てる——既定の blocking は配送が詰まるとアプリの書き出しを止めるため、ログの欠落よりアプリの停止を避ける側に倒す。これが守るのは配送の段までで、ドライバの初期化失敗（ロググループが無い・権限が無い・接続できない）はコンテナの起動失敗になる。ロググループと権限は IaC が先に作る。ログの上書きは prod の compose と別ファイルに置く。CI のコンテナ検査は AWS の資格情報を持たず、prod の compose をそのまま動かして本番の形を見ているため（34）。

**ホストのディスクとメモリだけは agent で送る。** どちらも EC2 の標準指標に無く、ディスクの逼迫は旧世代イメージの滞留（#336）を拾うために要る。agent の設定は Parameter Store に置いて起動時に読ませ、送る名前空間はインスタンスロールの条件で 1 つに限る。

**readiness は外から引かず、公開 URL の合成監視で代える。** `/q/*` は配信が `/api/*` しか流さないため、readiness の失敗を外から観測する経路が無い。代わりに Route53 のヘルスチェックで公開 Query の 1 経路（`/api/v1/albums`）を HTTPS で引く。この経路は CloudFront → WAF → オリジン識別 → backend → DB まで通るため、readiness が見るもの（DB 接続）を含んだうえで、静的サイトが読む API が外から使えることまで見る。切替前は配信のドメイン名、切替後は正規ドメインを引く（`dns_cutover_enabled` に従う）。Synthetics のカナリアは同じことを月に 10 倍近い費用で行うため採らない。

**通知先は SNS のトピックをリージョンごとに 1 つ、宛先は同じメール。** CloudWatch アラームのアクションはアラームと同じリージョンの SNS でなければならず、CloudFront と Route53 ヘルスチェックの指標は us-east-1 にしか無い。そのため主リージョンと us-east-1 に 1 つずつトピックを持つ。EventBridge や Lambda でリージョン間を転送して 1 トピックへ寄せる形は、部品と権限が増えるぶん採らない。復旧（OK）も同じ宛先へ通知し、止まったことと戻ったことの両方が残るようにする。宛先の値は運用側（tfvars）が持ち、トピックごとに届く購読の確認を踏むのは運用の手順。

**トレードオフ**: ダッシュボードと JVM 内部の指標（`/q/metrics`）の送出は持たない。要るときに、agent で scrape するか micrometer の別レジストリを足すかを決める。`user_data` の変更は既存インスタンスに効かず、初回構築の前に入れておく必要がある。ヘルスチェックの経路は公開 Query の 1 つに固定しているため、その経路の仕様変更（廃止・認証の付与）はここへ波及する。

## 資格情報の更新は tfvars の契機で生成し直し、直後に再配布する

管理APIキー・DBパスワード・オリジン識別値（#127）は、Terraform の `random_password` が生成元、Parameter Store（識別値は CloudFront の custom header も）が供給先、稼働中の backend が消費者という 3 点で成り立つ。更新はこの 3 点を揃える操作であり、どこか 1 点だけを触る経路を残すと、残りが黙って古いままになる。

**契機を tfvars の変数にし、`-replace` を手で打つ経路を使わない。** `random_password` の `keepers` に rotation 変数を結び、値を変えた apply で再生成する。変数の値は運用リポジトリの tfvars が持つため、いつ何を更新したかがそちらの履歴に残る。`terraform apply -replace` は同じことができるが、実行した事実がどこにも残らず、Parameter Store を手で書き換える経路と同じく「次の apply で戻る／戻らない」の判断を人に委ねる。

**apply の直後に再配布する。** `deploy.sh` は配布のたびに Parameter Store から読むため、消費者を新しい値へ揃える操作は「稼働中の commit を再配布する」で足りる（16 の手動経路。ビルドしない）。専用の「再起動だけ」の経路は作らない——配布と別の経路で動かすと、動いている手順と検査した手順が別のものになる。

**断は許容し、順序で短くする。** DBパスワードは RDS 側が即時に変わるため、apply から再配布までの間に新規接続が失敗しうる。オリジン識別値は CloudFront の反映と再配布が同時にならないため、一致しない間は `/api/*` が 403 になる（35）。どちらも新旧を同時に受け付ける改修で消せるが、受け付ける値が 2 つある状態を持つのは秘密の管理としては後退で、個人利用のサービスに数分の断を許容できる。apply の直後に再配布することと、1 つずつ行うことで、断を短く・原因を 1 つに保つ。

**戻す用の旧値を別に保管しない。** 「戻す」は「もう一度更新する」。生成値は Terraform の state に残るが、それは state の保護の話であって戻し方の話ではない。戻す経路として旧値を残せば戻せるが、残した旧値がどこで生きているかを追う負担が増える。管理APIキーの更新で全セッションが失効するのは 22 の帰結で、望む挙動。

**トレードオフ**: 更新の最中に手順どおりでない操作（Parameter Store の手書き、別の commit の配布）を挟むと、3 点のどれかが食い違う。それを機械で塞いではおらず、`infra/README.md` の順序と運用側の手順に委ねる。無停止の更新は持たない。

## 復旧は別インスタンスへ復元し、接続先の切り替えで戻す

復旧の単位は 6 つある（backend のイメージ、Flyway の版、public / admin の成果物、公開データの世代、DB の内容、画像。#130）。前の 4 つは 16・#125・#400 で既に戻し方を持つ。ここで決めるのは DB の内容と画像。

**DB は常設のインスタンスを上書きせず、別のインスタンスへ復元して接続先を切り替える。** 復元点は tfvars（`db_restore_to_time` か `db_restore_snapshot_identifier`）に置き、`aws_db_instance.restored` を一時のインスタンスとして作る。内容を確かめてから `db_active` で Parameter Store の接続先を切り替え、稼働中の commit を再配布する（資格情報の更新と同じ順序）。常設へ戻すときは、復元済みのスナップショットから常設を置換する。

**なぜ**: 復元先を常設にすると、復元を始めた時点で「いまの DB」が消える。復元点の選び直しも、復元が失敗したときの立ち戻りもできない。別のインスタンスなら、いまの DB を消さずに復元点を試し、確かめてから切り替えられる。接続先の切り替えを Parameter Store の手書きでなく tfvars に置くのは、資格情報の更新と同じ理由——手で書いた値は次の apply で戻り、記録も残らない。復元点や接続先が tfvars（運用リポジトリ）にあれば、いつ何をしたかがそちらの履歴に残る。

**常設は削除保護で守る。** `deletion_protection` を既定で true にし、置換や破棄は明示的に false にしてからでなければ通らない。復元の手順そのものが `-replace` を含むため、「いつもの apply」と「常設を作り直す apply」を同じ手つきにしない。

**画像は追記のみとし、backend が消せるのは受け入れ前だけにする。** 確定した画像（`assets/`）はキーが一意で上書きも無く、コードも消さない。それを権限でも固定する（`s3:DeleteObject` は `pending/*` だけ）。過去の時点へ戻した DB が参照する画像は必ず在り、復元より後に入った画像は参照されないまま残るだけで害が無い。したがって画像側に「DB の復元点に合わせて戻す」操作は無く、保持期間も設けない（DB の復元窓より短い保持を作らない）。

**撤回済みの内容の再公開は機構で防がない。** 公開データの世代は不透明な値で大小比較しないため（#125）、過去へ戻した DB でも `rebuild-public` は動く。復元で「公開」に戻った撤回済みの内容は、再ビルドの前に管理画面で人が確かめる。世代に順序を持たせて機械で止める案は、世代の契約（大小比較しない）を崩すため採らない。

**トレードオフ**: 接続先の切り替えから再配布までは断になる（無停止は持たない）。常設へ戻す工程は置換を含むため手数が多く、復元済みのインスタンスを長く接続先にしたままの状態も許す。クロスリージョンの複製は v1.0 に入れず、1 リージョンの喪失は受け入れる。RPO は任意時点復元の粒度（5 分程度）、RTO は復元と再配布の所要で決まり、目標値と実測は運用側が持つ。

**実体**: `infra/data.tf`（`aws_db_instance.main` の `deletion_protection` / `snapshot_identifier`、`aws_db_instance.restored`、`aws_ssm_parameter.db_host`）、`infra/compute.tf`（`AssetPendingDelete`）、`infra/tests/recovery.tftest.hcl`、`infra/README.md`「バックアップと復旧」。
