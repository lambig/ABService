...
管理画面のCSSを公開レイアウトへ混ぜないため、表示先を同一オリジンの空のiframeとする。親だけが管理APIを読み、描画済みHTMLを渡す。iframeのsandboxではスクリプトを許可せず、リンク遷移を親から止める。下書きや認証情報をURL・静的成果物に入れる経路と、プレビュー専用の公開APIは作らない。
