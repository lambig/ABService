...
**ログは agent ではなく Docker の awslogs ドライバで運ぶ。** prod の backend は JSON を stdout へ出しており、ドライバがそのまま 1 行 1 イベントで CloudWatch Logs へ送れる。agent にログも任せるとホストのファイルを経由する段が増え、ローテーションと権限の設定が要る。ドライバは `non-blocking` にし、宛先へ届かない間はバッファに溜めて溢れたら捨てる——既定の blocking はログの宛先が落ちるとアプリの書き出しを止めるため、ログの欠落よりアプリの停止を避ける側に倒す。ログの上書きは prod の compose と別ファイルに置く。CI のコンテナ検査は AWS の資格情報を持たず、prod の compose をそのまま動かして本番の形を見ているため（34）。
...
**通知先は SNS の 1 トピック、宛先はメール。** 復旧（OK）も同じ宛先へ通知し、止まったことと戻ったことの両方が残るようにする。宛先の値は運用側（tfvars）が持ち、購読の確認を踏むのは運用の手順。
