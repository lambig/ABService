...
通知先は SNS のトピック1つで、宛先は `alarm_email`（`terraform.tfvars`。リポジトリに書かない）。**購読の確認メールを踏むまで届かない。** 復旧（OK）も同じ宛先へ通知する。閾値は `variables.tf` の既定を持ち、`tfvars` で上書きできる。
...
`user_data` の変更（agent の導入）は既存インスタンスには効かない（初回起動時にしか走らない）。`awslogs` ドライバは宛先へ届かない間はバッファに溜めて溢れたら捨てる（`mode: non-blocking`）。ログの欠落よりアプリの停止を避ける側に倒している。
