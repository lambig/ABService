...
計画作成時の作品照会は1件ずつ行う。入力件数に比例した同時照会でAPI前段の同時実行制限に達することを避けるため。
照会に失敗した場合は計画を完成させず停止し、書き込みへ進まない。
...
本文ファイルの末尾の改行は1つだけ落とす（編集器が付ける改行を内容にしない）。空（空白と改行だけ）のファイルは
誤り。項目を省くならファイルを置かない。
...
知らない項目（綴り違い）、必須項目の欠け、空文字（JSON の項目も別ファイルの本文も）、片方しか置けないファイルの
重複、無い画像、受け入れない画像形式、同じタイトルの記事、解けない作品参照は、どのファイルのどの項目かを示して
止まる。値の妥当性
...
これが成り立つのは、投入先に残るものが**完全なもの**だけだから。作品は画像を確定してから1リクエストで作る。
記事は本体・作品参照・タグが別々のリクエストのため、本体の後で落ちたときは作った記事を消してから失敗を伝える
（消せなかったときは記事の ID を示す。手で消してから再実行する）。参照やタグを欠いた下書きが「登録済み」として
残り、欠けたまま公開される経路を作らないため。
