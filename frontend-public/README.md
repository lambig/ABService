...
色・タイポグラフィ・角丸の定義は `packages/public-presentation/src/public.css` にあり、`src/styles/global.css` から読み込む。Tailwind の既定パレットは落としてあり、`bg-blue-500` のような直指定はクラスが生成されない。見た目を変えるときはこのファイルのトークンを差し替える。
