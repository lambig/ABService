...
**中間状態で整合が取れない変更は1つのPRにまとめる。** 型構成の変更・永続化の追従・外向き契約の追従・旧実装の撤去が互いに依存する場合、途中で切るとコンパイルもマイグレーションも通らない状態を審査させることになり、レビューは容易にならない。

**一方、中心テーマから独立して成立する変更は別PRへ分ける。** 隣接して見つかった項目の整理（nullable の見直し、値オブジェクト化、命名の統一など）は、中心の構造変更が完了してからでも単独で成立する。同一PRへ載せるとレビュー対象の面積だけが増える。

分けるかどうかは「そのPRだけを取り出して `check` が通り、意味のある状態になるか」で判断する。
...
- パフォーマンスへの影響
- セキュリティ要件の遵守
- 壊れやすい経路が実DB統合テストで固定されているか（永続化の分岐・カスケード・子行の削除など、型検査で守れない箇所）
