 ...
 * {@link CreateAlbumArticleService} が {@code Result} 経由で行います。
 * </p>
 *
 * <p>
 * 入手経路（{@code acquisitionChannels}）の追加は {@code addAcquisitionChannel} で別途行うため、
 * 本コマンドの対象外です（横展開フェーズで別コマンドとして実装）。
 * </p>
 *
 * @param albumId
 *            対応するAlbum集約のID（必須。既存のAlbumを参照）
 * @param introLong
 ...
 *            頒布情報（nullable）
 */
public record CreateAlbumArticleInput(
